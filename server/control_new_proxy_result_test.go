...
func TestHandleNewProxyRefreshesHeartbeatBeforeSynchronousPlugin(t *testing.T) {
	...
	// v0.71.0 moved SessionContext.Conn to *msg.Conn; wrap the tracking conn so
	// close tracking still observes the underlying transport.
	trackedMsgConn := msg.NewConn(trackedConn, msg.NewV1ReadWriter(trackedConn))
	t.Cleanup(func() {
		_ = trackedConn.Close()
		_ = peerConn.Close()
	})
	...
	ctl := &Control{
		serverMetrics: servermetrics.Server,
		...
		msgDispatcher: msg.NewDispatcher(trackedMsgConn),
	...
	// v0.71.0 made heartbeatWorker synchronous (worker() now calls it with go),
	// where the v0.68.1 version spawned its own goroutine. Match production's
	// call shape so this still probes the refresh window rather than blocking
	// into the heartbeat timeout.
...
func TestCloseProxyKeepsOldAttemptIDWhenReplacementUsesSameIdentity(t *testing.T) {
	...
	newControl := func() *Control {
		return &Control{serverMetrics: servermetrics.Server, sessionCtx: &SessionContext{
			PxyManager:    proxy.NewManager(),
			PluginManager: manager,
